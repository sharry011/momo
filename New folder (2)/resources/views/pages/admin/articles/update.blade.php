...
@section('title', 'تعديل معلومات المقال')

@section('content')
    <form class="row p-0 m-0 my-4" id="new_post_form"
          action="{{url(route('admin.articles.update.post', ['id' => $post->id]))}}" method="post"
          enctype="multipart/form-data">
        <input type="hidden" name="_token" value="{{ csrf_token() }}"/>

        <div class="first_title col-12">
            <h3>تعديل معلومات المقال</h3>
        </div>
        ...
        <div class="col-12 col-lg-8">
            ...
            <div class="cont">
                ...
                <div class="details">
                    ...
                    <div class="mb-3 row">
                        <label for="" class="col-sm-2 col-form-label">النوع</label>
                        <div class="col-sm-10">
                            <input type="text" readonly class="form-control-plaintext"
                                   value="@if($post->opt == 1)فيلم@elseif($post->opt == 3)عرض@elseif($post->opt == 2)حلقة@endif"/>
                            <input name="opt" value="{{$post->opt}}" hidden/>
                        </div>
                    </div>

                    <div class="mb-3 row" id="ep_num-container" @if($post->opt != 2)style="display: none;"@endif>
                        <label for="" class="col-sm-2 col-form-label">رقم الحلقة</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" value="{{$post->num}}" name="ep_num"/>
                        </div>
                    </div>
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control" value="{{$post->runtime}}" name="runtime"/>
                        </div>
                    </div>
                    ...
                    <div class="mb-3 row">
                        <label for="sil" class="col-sm-2 col-form-label">الملصق</label>
                        <div class="col-sm-10 d-flex gap-1">
                            <input type="text" class="form-control" name="sticker_text"
                                   value="{{$post->sticker_text}}"/>
                            <input type="color" class="form-control form-control-color" id="" name="sticker_color"
                                   value="{{$post->sticker_color}}"/>
                        </div>
                    </div>
            ...
            <div class="cont">
                ...
                <div class="details">
                    ...
                    <textarea class="w-100 mb-2"
                              name="watch_servers">{{ implode("\n", $post->watch_servers) }}</textarea>
                    <hr/>
                    <label for="" class="form-label"> سيرفرات تحميل 1080 :</label>
                    <textarea class="w-100 mb-2"
                              name="post_down_servers_1080">{{ implode("\n", array_column(array_filter($post->down_servers, function($item) { return $item['size'] == 1080; }), 'code')) }}</textarea>
                    <label for="" class="form-label"> سيرفرات تحميل 720 :</label>
                    <textarea class="w-100 mb-2"
                              name="post_down_servers_720">{{ implode("\n", array_column(array_filter($post->down_servers, function($item) { return $item['size'] == 720; }), 'code')) }}</textarea>
                    <label for="" class="form-label"> سيرفرات تحميل 480 :</label>
                    <textarea class="w-100 mb-2"
                              name="post_down_servers_480">{{ implode("\n", array_column(array_filter($post->down_servers, function($item) { return $item['size'] == 480; }), 'code')) }}</textarea>


                    <label for="" class="form-label d-none"> سيرفرات تحميل متعددة الجودات :</label>
                    <textarea class="w-100 mb-2 d-none"
                              name="post_down_servers_multi">{{ implode("\n", array_column(array_filter($post->down_servers, function($item) { return $item['size'] == 10000; }), 'code')) }}</textarea>
                    <label for="" class="form-label d-none"> سيرفرات تحميل 360 :</label>
                    <textarea class="w-100 mb-2 d-none"
                              name="post_down_servers_360">{{ implode("\n", array_column(array_filter($post->down_servers, function($item) { return $item['size'] == 360; }), 'code')) }}</textarea>
                </div>
            </div>
            ...
            <div class="cont">
                ...
                <div class="details">
                    ...
                    <div class="form-check form-switch py-2">
                        <input class="form-check-input fs-4 m-0 mt-1" type="checkbox" role="switch" id="pinned"
                               name="pin" @if($post->pin_index) checked @endif />
                    ...
                    <div class="row g-3 align-items-center justify-content-between m-0">
                        ...
                        <div class="col-auto">
                            <input type="number" id="views" class="form-control" value="{{$post->views}}" name="views"/>
                        </div>
                    </div>
                </div>
            </div>
