                    ...
                    <tr>
                        ...
                        <td>
                            <div class="post_actions d-flex flex-row flex-nowrap">
                                <a href="{{route('admin.articles.copy_from_season.get', ['id' => $post->id])}}"
                                   class="btn btn-sm btn-primary br-0" title="نسخ الى مقال"><i
                                        class="fa-solid fa-photo-film"></i>
                                </a>
                                <a href="{{route('admin.seasons.copy.get', ['id' => $post->id])}}"
                                   class="btn btn-sm btn-success br-0"
                                   title="نسخ"><i class="fa-solid fa-copy"></i>
                                </a>
                                <a href="{{route('admin.seasons.update.get', ['id' => $post->id])}}"
                                   class="btn btn-sm btn-warning br-0"
                                   title="تعديل"><i class="fa-solid fa-pen-to-square"></i></a>

                                <form method="post" class="form_delete"
                                      action="{{route('admin.seasons.delete', ['page' => request()->query('page'), 's' => request()->query('s')])}}">
                                    @csrf
