    ...
    <div class="cont">
        <div class="row m-3">
            <div class="col-12 col-lg-4 p-2">
                ...
                <form class="details" action="{{route('admin.down_servers.new.post')}}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3">
            ...
            <div class="col-12 col-lg-8 p-2 dd">
                <form class="details" id="servers_order" action="{{route('admin.down_servers.update.post')}}" method="post">
                        ...
                        @foreach($servers as $server)
                            <li class="dd-item" data-id="{{$server->id}}">
                                ...
                                <div class="dd-handle">
                                    ...
                                    <div class="d-flex"> حذف : &nbsp; <div class="ltr">{{$server->remove}}</div></div>
                                    <div class="d-flex"> اضافة : &nbsp; <div class="ltr">{{$server->add}}</div></div>
                                </div>
                            </li>
                        @endforeach
                    </ol>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#servers_order').on('submit', function(e) {
                ...
                var data = $('.dd').nestable('serialize');
                $.ajax({
                    url: '{{route('admin.down_servers.update.post')}}',
                    type: 'post',
                    data: {
                        _token: '{{csrf_token()}}',
                        data: data
